...
for batchN in batchList:
    ...
    dataExA = dataEx.loc[dataEx['batchID'] == batchN]
    for angleSeqN in angleSeq:
        justAng = dataExA.loc[dataExA['angleSeq'] == angleSeqN]
        ...
        for fstFrameN in FrameStim:
            ...
            d = {'batchList':batchN,'angleSeq': angleSeqN, 'fstFrameN': fstFrameN,'percentS': percentS, 'Observer': b, 'approxMs':approxMS, 'totalNo':len(sizeAng)} #"timebefore_mean": timebefore_mean, 'Resp_mean': Resp_mean, 'frameStim_mean':frameStim_mean
...
fI=1
for batchN in results.batchList:
