...
directoryName = 'tlTiming'

os.chdir(directoryName )


##loads up the spreadsheet
baseName ='tlTimingallFilesCol'
b = baseName + '.csv'
...
angleSeq= dataEx['angleSeq'].unique()
angleSeq = np.sort(angleSeq)

fI =1
columnNames = list(dataEx.columns.values)
...
for angleSeqN in angleSeq:
    ...
    for fstFrameN in FrameStim:
        ...
        d = {'angleSeq': angleSeqN, 'fstFrameN': fstFrameN,'percentS': percentS, 'Observer': b, 'approxMs':approxMS, 'totalNo':len(sizeAng)} #"timebefore_mean": sizeAng.timebefore_mean, 'Resp_mean': Resp_mean, 'frameStim_mean':frameStim_mean
...
for angleSeqN in angleSeq:
    ...
    plt.xticks([0.25, 0.5, 1, 1.25,1.5 ])
    ...
    plt.savefig(baseName+ advName+ 'angleSeq')
...
f = baseName + advName+ '_PSEresults.csv'     
PSEResults.to_csv(f)        
